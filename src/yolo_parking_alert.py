<<<<<<< ours
"""YOLO-based human detection with proximity alert for parking enforcement staff.

This script runs a YOLO model (default: ultralytics/yolov8n) on a video stream,
tracks people in the frame, and raises an audible/visual alert when a detected
person enters a predefined alert zone near your vehicle.
=======
...
>>>>>>> theirs
    ...
    def _trigger_alert(self) -> None:
        ...
        sys.stdout.flush()
<<<<<<< ours
        print("⚠️  Parking officer detected near your vehicle!", flush=True)
        self._last_alert_at = now

    def _parse_person_detections(self, results) -> Iterable[Detection]:
=======
...
<<<<<<< ours
                if cls_id != PERSON_CLASS_ID or conf < self.score_threshold:
                    continue
                x1, y1, x2, y2 = map(int, box.xyxy[0].tolist())
                yield Detection((x1, y1, x2, y2), conf, "person")

    def process_frame(self, frame):
        results = self.model(frame, verbose=False)
        person_detections = list(self._parse_person_detections(results))
        alert_needed = False
=======
